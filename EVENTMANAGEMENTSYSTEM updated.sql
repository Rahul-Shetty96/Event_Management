...
--FUNCTION
--2.Create function which takes classname,societyname as 
--parameter and returns count of members in that class
Create function Class_Comm_count (@classname as varchar(50),
@soc_name as varchar(50))
returns int 
as 
begin
declare @c as int
SELECT @c=count(*) FROM
				...
				INTERSECT
				(Select Email_id from SOCIETY_ALL 
				where  Society_name = @soc_name)) as temp(email)
return @c
end
				      
Select * from  SOCIETY_ALL
...
Select * from SOCIETY_ALL
Select * from STUDENT
print dbo.Class_Comm_count('D15','CSIVESIT')

Select * from SOC_NORMAL_MEMBER
Select * from COMMITTEE
Select * from STUDENT

Create view SOCIETY_ALL(Society_name , Email_id) as 
(Select society_name,email from SOC_NORMAL_MEMBER) 
UNION
(Select society_name,email from SOC_REPRESENTATIVE) 


2)Display society name having 
more number of members than members than CSI in class D15
  
 Select * from SOCIETY_ALL 
 Select Society_name from SOCIETY_ALL group  by Society_name having COUNT(*)>
 dbo.Class_Comm_count('D15','CSIVESIT') 
 
 

--1) Display student details of students who participates
         -- in event "MATH LOGIC" conducted by ISTE

SELECT * from student as s , where email in
(
select email from PARTICIPATES where 

)


-- 2) Display the details of society representive of CSI who is responsible 
           --for conducting workshop on python whose speaker is 'XYZ'
           
           
      
           
- 3) display society names and the 
number of students belonging to that society for D15


				
          

		select Society_name,COUNT(*)as No_of_Students
		from SOCIETY_ALL
		where Email_id in ()
...
--procedure
-- 3)create procedure which displays
-- number of society student belongs to when email id is passed


alter procedure mem_of_soc (@email as varchar(50))
as
begin 
declare @c as int
select @c=COUNT(*)  from SOCIETY_ALL
where Email_id = @email
print 'The number of committees to which '+@email+' belongs to is  '+cast(@c as varchar)
end
EXECUTE mem_of_soc 'rahul.shetty@ves.ac.in'


-- create procedure which displays the revenue of COMMITTEE
--  generated by the membership amount 

alter procedure calc_revenue(@comname as varchar(50))
as
begin
declare @cost as int,@c as int
select @cost = mem_amt from SOCIETY where soc_name = @comname

Select @c = COUNT(*)
from SOCIETY_ALL 
where Society_name = @comname
set @c = @c*@cost
print 'The total revenue of '+@comname+' is '+cast(@c as varchar)
end

execute calc_revenue 'ISTEVESIT'
